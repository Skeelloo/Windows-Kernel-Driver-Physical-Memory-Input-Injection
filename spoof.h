...
#ifdef _KERNEL_MODE
	__forceinline ULONG RandomPoolTag() {
		static const ULONG tags[] = {
			'cmNt', 'CcSc', 'MmSt', 'ObDi', 'SeTi',
			'FMfn', 'AlEp', 'NtFi', 'IoNm', 'ExHd',
			'MmCi', 'CcBc', 'FMfl', 'NtFs', 'SePr'
		};
		return tags[(__rdtsc() >> 4) % (sizeof(tags) / sizeof(tags[0]))];
	}
	__forceinline PVOID LocateShellCode(PVOID func, size_t size = 500)
	{
		void* addr = ExAllocatePoolWithTag(NonPagedPool, size, RandomPoolTag());
		if (!addr)
			return nullptr;
